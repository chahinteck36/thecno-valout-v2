  ...
  return (
    <div className="group flex flex-col bg-slate-900/90 hover:bg-slate-900 border border-slate-800 hover:border-cyan-500/40 rounded-2xl overflow-hidden shadow-xl transition-all duration-300 hover:shadow-cyan-950/40">
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between">
        <div>
          {/* Genuine Technical Specs Badges */}
          <div className="flex items-center justify-between text-xs text-slate-400 mb-2 gap-2">
            <div className="flex items-center gap-1.5 min-w-0">
              <span className="px-2 py-0.5 rounded-md bg-slate-800 text-cyan-300 text-[11px] font-bold truncate border border-slate-700">
                {template.framework || (template.type === 'blogger' ? 'Blogger XML' : 'WordPress 6.x')}
              </span>
              {template.version && (
            ...
            <span className="text-[10px] text-emerald-400 font-bold flex items-center gap-1 flex-shrink-0">
              <CheckCircle2 className="w-3 h-3" />
              <span>{isEn ? 'Instant Delivery' : 'تسليم فوري'}</span>
            </span>
          </div>
